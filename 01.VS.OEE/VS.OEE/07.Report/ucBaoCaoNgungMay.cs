...
namespace VS.OEE
{
    public partial class ucBaoCaoNgungMay : DevExpress.XtraEditors.XtraUserControl
    {
        public ucBaoCaoNgungMay()
        {
            InitializeComponent();
            Commons.Modules.ObjSystems.ThayDoiNN(this);
        }
        private void ucBaoCaoNgungMay_Load(object sender, EventArgs e)
        {
        ...
        private void Loadgrdata()
        {
            ...
            try
            {
                dtmp.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, "spGetListDownTimeBC", Commons.Modules.UserName, Commons.Modules.TypeLanguage, cboMay.EditValue, datTuNgay.DateTime, datDenNgay.DateTime));
                Modules.ObjSystems.MLoadXtraGrid(grdBCNgungMay, grvBCNgungMay, dtmp, false, true, true, true, this.Name);
                Commons.Modules.ObjSystems.AddCombDateTimeEdit("TU_GIO", grvBCNgungMay);
                Commons.Modules.ObjSystems.AddCombDateTimeEdit("DEN_GIO", grvBCNgungMay);
        ...
        private void InDuLieu()
        {
            ...
            Excel.Range title;
            int TCot = grvBCNgungMay.Columns.Count;
            int TDong = grvBCNgungMay.RowCount;
            int Dong = 1;
            excelApplication.Visible = false;
            grvBCNgungMay.ActiveFilter.Clear();
            grvBCNgungMay.ExportToXlsx(sPath);
