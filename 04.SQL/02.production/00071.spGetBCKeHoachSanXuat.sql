...
AS
BEGIN 
SELECT DISTINCT  B.MS_MAY,C.ItemCode,A.ItemName,C.Barcode,AVG(B.StandardOutput) AS PPH,CONVERT(DATE,B.PlannedStartTime) AS NGAY
...
GROUP BY B.MS_MAY,C.ItemCode,A.ItemName,C.Barcode,CONVERT(DATE,B.PlannedStartTime) 
ORDER BY NGAY



SELECT DISTINCT MS_MAY,ItemCode,ItemName,Barcode,PPH,NGAY,1 AS ID_Resulst,'Planned' AS Name_Resulst,ISNULL(Planned,0) AS Resulst INTO #TMP1 FROM #TMP
UNION
SELECT DISTINCT MS_MAY,ItemCode,ItemName,Barcode,PPH,NGAY, 2 AS ID_Resulst,'Actual' AS Name_Resulst,Actual AS Resulst FROM #TMP
UNION
SELECT DISTINCT MS_MAY,ItemCode,ItemName,Barcode,PPH,NGAY,3 AS ID_Resulst,'Differences' AS Name_Resulst,Differences AS Resulst FROM #TMP
UNION
SELECT DISTINCT MS_MAY,ItemCode,ItemName,Barcode,PPH,NGAY, 4 AS ID_Resulst,'EFF' AS Name_Resulst,EFF AS Resulst FROM #TMP
ORDER BY  ItemCode,Barcode,MS_MAY,PPH,ID_Resulst,NGAY


SELECT MS_MAY,ItemCode,ItemName,C.Barcode,PPH,ID_Resulst,CASE WHEN ID_Resulst < 4 
...
GROUP BY MS_MAY,ItemCode,ItemName,C.Barcode,PPH,ID_Resulst
...
(SELECT MS_MAY,ItemCode,ItemName,Barcode,PPH,CONVERT(NVARCHAR(10),NGAY,103) AS NGAY,ID_Resulst,Name_Resulst,Resulst FROM #TMP1
)
 A
		PIVOT
		(
		...
		SELECT A.MS_MAY,A.ItemCode,A.ItemName,A.Barcode,A.PPH,A.Name_Resulst,'+@sSql+',  convert(nvarchar(50),TONG) as TONG  FROM #TMP2 A
			INNER JOIN #TMP3 B ON B.MS_MAY = A.MS_MAY AND B.ItemCode = A.ItemCode AND B.ID_Resulst = A.ID_Resulst AND A.PPH = B.PPH ORDER BY A.MS_MAY,A.ItemCode,A.Barcode,A.PPH,A.ID_Resulst
'
               EXEC(@sSqlL)
