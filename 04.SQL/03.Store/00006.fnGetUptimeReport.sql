...
CREATE FUNCTION [dbo].[fnGetUptimeReport]
(
...
begin
	...
	GROUP BY B.MS_MAY
	--cách tính 1 : Planned production time - pland dowtime
	...
	WHERE (A.TU_GIO BETWEEN @TuNgay AND @DenNgay) AND (A.DEN_GIO BETWEEN @TuNgay AND @DenNgay) AND MS_MAY = @MS_MAY AND (B.Planned = @pland OR @pland = -1))
